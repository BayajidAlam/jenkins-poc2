...
**Phase B — create the nodes via UI, then grab their secrets.**

Repeat the block below **twice**, once for `agent-build` and once for `agent-push`.

| Step | Action |
|---|---|
| 1 | In Jenkins, click **Manage Jenkins → Nodes** (left sidebar). |
| 2 | Click **New Node** (top-right or "create new node" link). |
| 3 | Enter the node name (`agent-build` for the first run, `agent-push` for the second), pick **Permanent Agent**, click **Create**. |
| 4 | Fill in the configure screen: **Remote root directory** = `/home/jenkins/agent`; **Labels** = `build docker` (or `push docker` for the second); **Launch method** = *Launch agents via JNLP*; **Availability** = *Keep this agent online as much as possible*. Click **Save**. |
| 5 | You land on the node's page. **The JNLP secret is right here** — scroll to the *Agent password* / *JNLP* section. You'll see either a copyable hex string labeled *Agent password*, or a launch command line that contains `… -secret <hex> …`. **Copy that hex string.** |
| 6 | Alternative path: open `http://<host>:8080/computer/agent-build/slave-agent.jnlp` in a browser (substitute `agent-push` for the second one). The XML body has `<argument>some-hex-string</argument>` as the **first** `<argument>` inside `<application-desc>` — that hex string is the JNLP secret. (If Jenkins asks you to log in first, do that, then reload the URL.) |
| 7 | Put the secret in `.env`: for `agent-build` → `JENKINS_AGENT_BUILD_SECRET=<paste>`; for `agent-push` → `JENKINS_AGENT_PUSH_SECRET=<paste>`. |

After both are in `.env`:

```bash
docker compose up -d
```

Within ~30 seconds, both agent containers should connect and show as **online**
on *Manage Jenkins → Nodes*. If a node stays offline, your pasted secret doesn't
match the controller - go back to step 5 and re-copy.

**CLI alternative to step 5** (faster than opening the UI): the JNLP secret
for each node is stored as a hashed XML file inside the controller's volume.
Read it directly:
...
Each prints a one-liner like `<secret>9f3c…e2</secret>`. The hex string inside
`<secret>…</secret>` is the value to paste into `.env`. No login required.
